      ...
      <div className="flex flex-col gap-y-2">
        {
          shareMenuItems.map((item, idx) => {
            ...
            return <Button variant="outline" onClick={onButtonClicked} key={`share-${idx}`}>{item.title}</Button>
          })
        }
      </div>
    </DialogContent>
...
const TopRightButtons: React.FC<Props> = (props) => {
  ...
  return (
    <Card className="w-full h-12 border-0 rounded-t-[12px] rounded-b-[0] backdrop-blur-[2px]">
        ...
        <div>
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              {combinedInformation.directMessageItems.map((item, idx) => {
                const onDropdownItemClicked = () => {
                  window.open(item.link, "_blank");
                };

                return (
                  <DropdownMenuItem onClick={onDropdownItemClicked} key={`topBtn-${idx}`}>
                    {item.name}
                  </DropdownMenuItem>
                );
              })}
